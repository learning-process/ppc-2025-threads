...
namespace {
void GenerateRandomData(std::vector<alputov_i_graham_scan_stl::Point>& data, size_t count) {
  std::mt19937 gen(42);
...
void ValidateTask(alputov_i_graham_scan_stl::TestTaskSTL& task) {
...
std::vector<alputov_i_graham_scan_stl::Point> GenerateStarPoints(size_t num_points_star) {
  std::vector<alputov_i_graham_scan_stl::Point> input;
...
void AssertConvexHullSize(const std::vector<alputov_i_graham_scan_stl::Point>& convex_hull,
                          const std::vector<alputov_i_graham_scan_stl::Point>& input, size_t expected_min_size) {
  EXPECT_LE(convex_hull.size(), input.size());
  EXPECT_GE(convex_hull.size(), expected_min_size);
}

void AssertStarConvexHullPoints(const std::vector<alputov_i_graham_scan_stl::Point>& convex_hull,
                                size_t num_points_star) {
  std::set<alputov_i_graham_scan_stl::Point> hull_set(convex_hull.begin(), convex_hull.end());
...
void ValidateStarConvexHull(alputov_i_graham_scan_stl::TestTaskSTL& task,
                            const std::vector<alputov_i_graham_scan_stl::Point>& input, size_t num_points_star) {
  ValidateTask(task);
...
TEST(alputov_i_graham_scan_stl, minimal_triangle_case) {
  std::vector<alputov_i_graham_scan_stl::Point> input = {{0, 0}, {2, 0}, {1, 2}};
  std::vector<alputov_i_graham_scan_stl::Point> output(3);
...
TEST(alputov_i_graham_scan_stl, collinear_points) {
  std::vector<alputov_i_graham_scan_stl::Point> input = {{0, 0}, {1, 1}, {2, 2}, {3, 3}};
...
TEST(alputov_i_graham_scan_stl, perfect_square_case) {
  std::vector<alputov_i_graham_scan_stl::Point> input = {{0, 0}, {0, 5}, {5, 5}, {5, 0}};
  std::vector<alputov_i_graham_scan_stl::Point> output(input.size());
...
TEST(alputov_i_graham_scan_stl, random_1000_points) {
  std::vector<alputov_i_graham_scan_stl::Point> input;
...
TEST(alputov_i_graham_scan_stl, duplicate_points) {
  std::vector<alputov_i_graham_scan_stl::Point> input(10, {2.5, 3.5});
  ...
  EXPECT_FALSE(task.GetConvexHull().empty());
  const auto& convex_hull = task.GetConvexHull();
  std::set<alputov_i_graham_scan_stl::Point> unique_hull(convex_hull.begin(), convex_hull.end());
  EXPECT_EQ(unique_hull.size(), 4U);
}
TEST(alputov_i_graham_scan_stl, convex_polygon) {
  std::vector<alputov_i_graham_scan_stl::Point> input = {{0, 0}, {4, 0}, {4, 4}, {2, 6}, {0, 4}};
  std::vector<alputov_i_graham_scan_stl::Point> output(input.size());
...
TEST(alputov_i_graham_scan_stl, concave_polygon) {
  std::vector<alputov_i_graham_scan_stl::Point> input = {{0, 0}, {4, 0}, {4, 4}, {2, 2}, {0, 4}};
  std::vector<alputov_i_graham_scan_stl::Point> output(input.size());
...
TEST(alputov_i_graham_scan_stl, regular_polygon) {
  std::vector<alputov_i_graham_scan_stl::Point> input = {{0, 0}, {2, 0}, {3, 1}, {2, 2}, {0, 2}, {-1, 1}};
  std::vector<alputov_i_graham_scan_stl::Point> output(input.size());
...
TEST(alputov_i_graham_scan_stl, circle_figure) {
  std::vector<alputov_i_graham_scan_stl::Point> input;
