...
TEST(lavrentiev_a_ccs_stl, test_pipeline_run) {
  constexpr double kSize = 700;
  auto task = TestData({kSize, kSize}, {kSize, kSize}, 6);
  auto test_task_stl = std::make_shared<lavrentiev_a_ccs_stl::CCSSTL>(task.task_data_stl);
...
TEST(lavrentiev_a_ccs_stl, test_task_run) {
  constexpr double kSize = 700;
  auto task = TestData({kSize, kSize}, {kSize, kSize}, 6);
  auto test_task_stl = std::make_shared<lavrentiev_a_ccs_stl::CCSSTL>(task.task_data_stl);
  auto perf_attr = std::make_shared<ppc::core::PerfAttr>();
