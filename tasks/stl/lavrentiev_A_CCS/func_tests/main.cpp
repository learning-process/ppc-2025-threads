...
std::vector<double> GenerateRandomMatrix(int size, int sparse_size) {
  ...
  std::uniform_int_distribution<> random_element(-5000, 10000);
...
struct TestData {
  ...
  std::shared_ptr<ppc::core::TaskData> task_data_stl;
  TestData(std::pair<int, int> matrix1_size, std::pair<int, int> matrix2_size, int sparse_size);
  [[nodiscard]] lavrentiev_a_ccs_stl::CCSSTL CreateTask() const;
};
...
TestData::TestData(std::pair<int, int> matrix1_size, std::pair<int, int> matrix2_size, int sparse_size) {
  ...
  task_data_stl = std::make_shared<ppc::core::TaskData>();
  task_data_stl->inputs.emplace_back(reinterpret_cast<uint8_t *>(random_data.data()));
  task_data_stl->inputs.emplace_back(reinterpret_cast<uint8_t *>(single_matrix.data()));
  task_data_stl->inputs_count.emplace_back(matrix1_size.first);
  task_data_stl->inputs_count.emplace_back(matrix1_size.second);
  task_data_stl->inputs_count.emplace_back(matrix2_size.first);
  task_data_stl->inputs_count.emplace_back(matrix2_size.second);
  task_data_stl->outputs.emplace_back(reinterpret_cast<uint8_t *>(result.data()));
  task_data_stl->outputs_count.emplace_back(result.size());
}

lavrentiev_a_ccs_stl::CCSSTL TestData::CreateTask() const { return lavrentiev_a_ccs_stl::CCSSTL(task_data_stl); }
}  // namespace

TEST(lavrentiev_a_ccs_stl, test_0x0_matrix) {
  std::vector<double> a;
  std::vector<double> b;
  std::vector<double> result;
  ...
  lavrentiev_a_ccs_stl::CCSSTL test_task_stl(task_data_stl);
...
TEST(lavrentiev_a_ccs_stl, test_3x2_matrix) {
  ...
  lavrentiev_a_ccs_stl::CCSSTL test_task_tbb(task_data_stl);
...
TEST(lavrentiev_a_ccs_stl, test_3x3_matrixes) {
...
TEST(lavrentiev_a_ccs_stl, test_12x12_matrix) {
  auto task = TestData({12, 12}, {12, 12}, 1);
  auto test_task_stl = task.CreateTask();
  ASSERT_EQ(test_task_stl.Validation(), true);
  test_task_stl.PreProcessing();
  test_task_stl.Run();
  test_task_stl.PostProcessing();
...
TEST(lavrentiev_a_ccs_stl, test_25x25_matrix) {
