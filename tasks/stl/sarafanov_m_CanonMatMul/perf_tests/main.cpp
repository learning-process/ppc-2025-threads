...
TEST(sarafanov_m_canon_mat_mul_stl, test_pipeline_run) {
  ...
  auto test_task_stl = std::make_shared<sarafanov_m_canon_mat_mul_stl::CanonMatMulOMP>(task_data_stl);
...
TEST(sarafanov_m_canon_mat_mul_stl, test_task_run) {
  constexpr size_t kCount = 250;
  constexpr double kInaccuracy = 0.001;
  auto a_matrix = GenerateRandomData(static_cast<int>(kCount * kCount));
  auto single_matrix = GenerateSingleMatrix(static_cast<int>(kCount * kCount));
