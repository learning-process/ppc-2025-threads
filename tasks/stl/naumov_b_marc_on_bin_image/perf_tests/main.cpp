...
TEST(naumov_b_marc_on_bin_image_stl, test_pipeline_run) {
  ...
  task_data_stl->inputs_count.emplace_back(kCo);
  task_data_stl->inputs_count.emplace_back(kCo);
  task_data_stl->outputs.emplace_back(reinterpret_cast<uint8_t *>(out.data()));
  task_data_stl->outputs_count.emplace_back(kCo);
  task_data_stl->outputs_count.emplace_back(kCo);

  auto test_task_stl = std::make_shared<naumov_b_marc_on_bin_image_stl::TestTaskSTL>(task_data_stl);
...
TEST(naumov_b_marc_on_bin_image_stl, test_task_run) {
