...
void zolotareva_a_sle_gradient_method_stl::TestTaskSTL::ConjugateGradient(const std::vector<double>& a,
  ...
  for (int iter = 0; iter <= n; ++iter) {
    ...
    // p = r + beta*p
    parallel_for(0, n, [&](int i) { p[i] = r[i] + beta * p[i]; });
...
double zolotareva_a_sle_gradient_method_stl::TestTaskSTL::DotProduct(const std::vector<double>& vec1,
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    ...
    futures.emplace_back(std::async(std::launch::async, [=, &vec1, &vec2] {
      double sum = 0;
      for (int i = s; i < e; ++i) sum += vec1[i] * vec2[i];
      return sum;
    }));
  }
  double total = 0;
...
bool zolotareva_a_sle_gradient_method_stl::TestTaskSTL::IsPositiveAndSimm(const double* a, int n) {
  ...
  for (int i = 0; i < n; ++i)
    for (int j = i + 1; j < n; ++j)
      if (m_[i * n + j] != m_[j * n + i]) return false;
  // проверка положительной определённости (холу разложение)
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j <= i; ++j) {
      double sum = m_[i * n + j];
      for (int k = 0; k < j; ++k) sum -= m_[i * n + k] * m_[j * n + k];
      if (i == j) {
        if (sum <= 1e-15) return false;
        m_[i * n + j] = std::sqrt(sum);
      } else {
        m_[i * n + j] = sum / m_[j * n + j];
      }
    }
  }
  return true;
...
void zolotareva_a_sle_gradient_method_stl::TestTaskSTL::parallel_for(int start, int end,
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    int s = start + t * block, e = std::min(end, s + block);
    if (s >= e) break;
    threads.emplace_back([s, e, &f] {
      for (int i = s; i < e; ++i) f(i);
    });
  }
