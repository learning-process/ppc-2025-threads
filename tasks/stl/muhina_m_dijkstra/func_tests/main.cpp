...
TEST(muhina_m_dijkstra_stl, test_dijkstra_small_graph) {
...
TEST(muhina_m_dijkstra_stl, test_dijkstra_validation_failure) {
  std::vector<int> graph_data;
  size_t start_vertex = 0;
...
TEST(muhina_m_dijkstra_stl, test_dijkstra_small_graph_non_zero_start) {
  constexpr size_t kNumVertices = 5;
  std::vector<std::vector<std::pair<size_t, int>>> adj_list(kNumVertices);
  adj_list[0].emplace_back(1, 4);
  adj_list[0].emplace_back(2, 2);
...
TEST(muhina_m_dijkstra_stl, test_negative_weight) {
  constexpr size_t kNumVertices = 5;
  std::vector<std::vector<std::pair<size_t, int>>> adj_list(kNumVertices);
  adj_list[0].emplace_back(1, -4);
  adj_list[0].emplace_back(2, 2);
...
TEST(muhina_m_dijkstra_stl, test_dijkstra_random_graph) {
...
TEST(muhina_m_dijkstra_stl, single_vertex_graph) {
...
TEST(muhina_m_dijkstra_stl, two_connected_components) {
...
TEST(muhina_m_dijkstra_stl, test_default_start_vertex) {
...
TEST(muhina_m_dijkstra_stl, test_graph_with_self_loops) {
  ...
  std::vector<int> expected_distances = {0, 2, 5};
  for (size_t i = 0; i < kNumVertices; ++i) {
    EXPECT_EQ(distances[i], expected_distances[i]);
  }
}