...
TEST(kolokolova_d_integral_simpson_method_stl, test_pipeline_run) {
  ...
  auto test_task_sequential =
      std::make_shared<kolokolova_d_integral_simpson_method_stl::TestTaskSTL>(task_data_stl, func);
...
TEST(kolokolova_d_integral_simpson_method_stl, test_task_run) {
  auto func = [](std::vector<double> vec) {
    return (vec[2] * vec[2] * vec[2] * vec[1] * vec[1] / 10) + (4 * vec[0] * vec[0]) - (10 * vec[2]);
  };
  std::vector<int> step = {130, 130, 130};
