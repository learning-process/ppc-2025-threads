...
TEST(vladimirova_j_m_monte_karlo_stl, test_pipeline_run) {
  // Create data
  double ans = 1;
  ...
  task_data_stl->inputs.emplace_back(reinterpret_cast<uint8_t*>(RandomFunc));
  task_data_stl->inputs.emplace_back(reinterpret_cast<uint8_t*>(2500000));
  ...
  // Create Task
  auto test_task_stluential = std::make_shared<vladimirova_j_m_monte_karlo_stl::TestTaskStl>(task_data_stl);
...
TEST(vladimirova_j_m_monte_karlo_stl, test_task_run) {
  // Create data
  double ans = 1;
  std::vector<double> val_b = GenRandomVectorArea(2, -10, 30, ans);
  std::vector<double> out(1, 0);
