...
TEST(zinoviev_a_convex_hull_components_stl, test_pipeline_run) {
  ...
  task_data->inputs_count.emplace_back(size);
  task_data->outputs.emplace_back(reinterpret_cast<uint8_t*>(new zinoviev_a_convex_hull_components_stl::Point[4]));
  task_data->outputs_count.emplace_back(4);

  auto task = std::make_shared<zinoviev_a_convex_hull_components_stl::ConvexHullSTL>(task_data);
