...
#ifndef _WIN32
TEST(varfolomeev_g_histogram_linear_stretching_all, test_opencv_image_validation) {
  // loading template orginal img
  cv::Mat input_image =
      cv::imread(ppc::util::GetAbsolutePath("all/varfolomeev_g_histogram_linear_stretching/data/cobble_orig.jpg"),
                 cv::IMREAD_GRAYSCALE);
  ASSERT_FALSE(input_image.empty());

  // loading template modified img
  cv::Mat reference_image =
      cv::imread(ppc::util::GetAbsolutePath("all/varfolomeev_g_histogram_linear_stretching/data/cobble_mod.jpg"),
                 cv::IMREAD_GRAYSCALE);
...
TEST(varfolomeev_g_histogram_linear_stretching_all, test_opencv_manual_64x64) {
  ...
  auto task_data_all = std::make_shared<ppc::core::TaskData>();
  task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t *>(input_vector.data()));
  task_data_all->inputs_count.emplace_back(input_vector.size());
  task_data_all->outputs.emplace_back(reinterpret_cast<uint8_t *>(output_vector.data()));
  task_data_all->outputs_count.emplace_back(output_vector.size());

  varfolomeev_g_histogram_linear_stretching_all::TestTaskALL test_task_all(task_data_all);
  EXPECT_TRUE(test_task_all.ValidationImpl());
...
TEST(varfolomeev_g_histogram_linear_stretching_all, test_manual_9) {
  // Create data
