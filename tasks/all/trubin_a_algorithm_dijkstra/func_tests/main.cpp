...
void RunDijkstraTest(const std::vector<int>& graph_data, int start_vertex, size_t vertex_count,
  ...
  task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t*>(const_cast<int*>(graph_data.data())));
  ...
  task_data_all->outputs_count.emplace_back(vertex_count);

  trubin_a_algorithm_dijkstra_all::TestTaskALL task(task_data_all);
...
TEST(trubin_a_algorithm_dijkstra_all, trivial_graph) { RunDijkstraTest({-1}, 0, 1, {0}); }

TEST(trubin_a_algorithm_dijkstra_all, linear_graph) {
  RunDijkstraTest({1, 2, -1, 2, 3, -1, 3, 1, -1, -1}, 0, 4, {0, 2, 5, 6});
}

TEST(trubin_a_algorithm_dijkstra_all, disconnected_graph) { RunDijkstraTest({1, 1, -1, -1, -1}, 0, 3, {0, 1, -1}); }

TEST(trubin_a_algorithm_dijkstra_all, cycle_graph) { RunDijkstraTest({1, 2, -1, 2, 2, -1, 0, 2, -1}, 0, 3, {0, 2, 4}); }

TEST(trubin_a_algorithm_dijkstra_all, fork_graph) { RunDijkstraTest({1, 1, 2, 5, -1, 2, 1, -1, -1}, 0, 3, {0, 1, 2}); }

TEST(trubin_a_algorithm_dijkstra_all, multiple_paths_same_cost) {
  RunDijkstraTest({1, 2, 2, 2, -1, 3, 2, -1, 3, 2, -1, -1}, 0, 4, {0, 2, 2, 4});
}

TEST(trubin_a_algorithm_dijkstra_all, unreachable_from_middle) {
  RunDijkstraTest({1, 1, -1, 2, 1, -1, -1, -1}, 0, 4, {0, 1, 2, -1});
}

TEST(trubin_a_algorithm_dijkstra_all, empty_graph) {
  std::vector<int> empty_graph;
  std::vector<int> expected;
  RunDijkstraTest(empty_graph, 0, 0, expected);
}

TEST(trubin_a_algorithm_dijkstra_all, invalid_start_vertex) {
  std::vector<int> graph = {1, 1, -1, -1};
  std::vector<int> out(2, -1);
  int start_vertex = 10;

  auto task_data_all = std::make_shared<ppc::core::TaskData>();
  task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t*>(graph.data()));
  task_data_all->inputs_count.emplace_back(graph.size());
  task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t*>(&start_vertex));
  task_data_all->outputs.emplace_back(reinterpret_cast<uint8_t*>(out.data()));
  task_data_all->outputs_count.emplace_back(2);
...
TEST(trubin_a_algorithm_dijkstra_all, negative_weight_should_fail) {
  std::vector<int> graph = {1, -1, -1, -1};
  std::vector<int> out(2, -1);
  int start_vertex = 0;
...
TEST(trubin_a_algorithm_dijkstra_all, validation_passes_on_empty_graph_and_outputs) {
  auto task_data_all = std::make_shared<ppc::core::TaskData>();
...
TEST(trubin_a_algorithm_dijkstra_all, validation_fails_on_empty_counts) {
  auto task_data_all = std::make_shared<ppc::core::TaskData>();
  task_data_all->inputs.emplace_back(nullptr);
  task_data_all->outputs.emplace_back(nullptr);
...
TEST(trubin_a_algorithm_dijkstra_all, validation_fails_on_empty_inputs) {
  auto task_data_all = std::make_shared<ppc::core::TaskData>();
  trubin_a_algorithm_dijkstra_all::TestTaskALL task(task_data_all);
  ASSERT_FALSE(task.Validation());
}

TEST(trubin_a_algorithm_dijkstra_all, multi_component_graph) {
  RunDijkstraTest({1, 1, -1, -1, 3, 2, -1, -1}, 0, 4, {0, 1, -1, -1});
}

TEST(trubin_a_algorithm_dijkstra_all, start_from_nonzero_vertex) {
  RunDijkstraTest({1, 3, -1, 2, 4, -1, 3, 2, -1, -1}, 2, 4, {-1, -1, 0, 2});
}

TEST(trubin_a_algorithm_dijkstra_all, edge_to_nonexistent_vertex_should_fail) {
  ...
  task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t*>(&start_vertex));
  task_data_all->outputs.emplace_back(reinterpret_cast<uint8_t*>(out.data()));
  task_data_all->outputs_count.emplace_back(3);
...
TEST(trubin_a_algorithm_dijkstra_all, random_large_graph) {
...
TEST(trubin_a_algorithm_dijkstra_all, complex_cycle_graph) {
