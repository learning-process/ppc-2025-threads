...
TEST(leontev_n_fox_all, 3x3_random) {
  size_t n = 3;
  ...
  ref_data = leontev_n_fox_all::MatMul(a, b, n);
  ...
  leontev_n_fox_all::FoxALL task(task_data);
...
TEST(leontev_n_fox_all, 111x111_random) {
  size_t n = 111;
  std::vector<double> in_data = GenerateRandomMatrix(2 * n * n, 666);
  std::vector<double> out_data(n * n);
  std::vector<double> ref_data(n * n);
