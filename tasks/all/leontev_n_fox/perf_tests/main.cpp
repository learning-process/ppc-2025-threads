...
TEST(leontev_n_fox_perf_all, test_pipeline_run) {
  size_t n = 233;
  ...
  auto test_task_sequential = std::make_shared<leontev_n_fox_all::FoxALL>(task_data_stl);
...
TEST(leontev_n_fox_perf_all, test_task_run) {
  size_t n = 233;
  std::vector<double> in_data = GenerateRandomMatrix(2 * n * n, 666);
  std::vector<double> out_data(n * n);
  auto task_data_stl = std::make_shared<ppc::core::TaskData>();
