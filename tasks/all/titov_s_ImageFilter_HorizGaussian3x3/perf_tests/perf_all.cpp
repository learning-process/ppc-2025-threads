...
TEST(titov_s_image_filter_horiz_gaussian3x3_all, test_pipeline_run) {
  ...
  task_data_all->outputs.emplace_back(reinterpret_cast<uint8_t *>(output.data()));
  task_data_all->outputs_count.emplace_back(output.size());

  auto test_task_all = std::make_shared<titov_s_image_filter_horiz_gaussian3x3_all::GaussianFilterALL>(task_data_all);
