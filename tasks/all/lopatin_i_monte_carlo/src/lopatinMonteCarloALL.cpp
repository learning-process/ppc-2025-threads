...
bool TestTaskAll::RunImpl() {
  ...
  // Объём области интегрирования (одинаковый для всех процессов)
  double volume = 1.0;
  for (size_t j = 0; j < d; ++j) {
    volume *= (integrationBounds_[(2 * j) + 1] - integrationBounds_[2 * j]);
  }

  // Распределение итераций между MPI-процессами
  ...
  // Локальные вычисления с OpenMP
  double local_sum = 0.0;
#pragma omp parallel reduction(+ : local_sum)
  {
    // Уникальный seed для каждого потока и процесса
...
#pragma omp for
  ...
  // Финализация результата на ранге 0
  if (world_rank == 0) {
