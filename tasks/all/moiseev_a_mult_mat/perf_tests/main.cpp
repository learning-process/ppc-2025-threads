...
TEST(moiseev_a_mult_mat_mpi, test_pipeline_run) {
  ...
  task_data_mpi->inputs_count.emplace_back(matrix_a.size());

  task_data_mpi->inputs.emplace_back(reinterpret_cast<uint8_t *>(matrix_b.data()));
  task_data_mpi->inputs_count.emplace_back(matrix_b.size());

  task_data_mpi->outputs.emplace_back(reinterpret_cast<uint8_t *>(matrix_c.data()));
  task_data_mpi->outputs_count.emplace_back(matrix_c.size());

  auto test_task_mpi = std::make_shared<moiseev_a_mult_mat_mpi::MultMatMPI>(task_data_mpi);
...
TEST(moiseev_a_mult_mat_mpi, test_task_run) {
  constexpr int kCount = 500;
