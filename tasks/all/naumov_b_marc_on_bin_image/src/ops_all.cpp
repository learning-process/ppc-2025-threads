...
bool naumov_b_marc_on_bin_image_all::TestTaskALL::PreProcessingImpl() {
    ...
    if(rank == 0) {
        ...
        input_image_ = std::vector<int>(
            reinterpret_cast<int*>(task_data->inputs[0]),
...
void naumov_b_marc_on_bin_image_all::TestTaskALL::UnionLabels(int label1, int label2) {
    ...
    if(root1 != root2) {
        if(root1 < root2) label_parent_[root2] = root1;
        else label_parent_[root1] = root2;
    }
}
...
bool naumov_b_marc_on_bin_image_all::TestTaskALL::PostProcessingImpl() {
    std::vector<int> local_result(
        output_image_.begin() + halo_ * global_cols_,
        output_image_.begin() + (local_rows_ + halo_) * global_cols_
    );
    ...
    boost::mpi::gather(world_, local_result.data(), local_rows_ * global_cols_, 
                      output_image_.data(), 0);
