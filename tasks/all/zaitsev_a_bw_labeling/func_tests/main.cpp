...
class zaitsev_a_labeling_test_mpi : public ::testing::TestWithParam<std::string> {
 protected:
#ifndef _WIN32
  ...
  static TestData PrepareImages(const std::string& filename) {
    cv::Mat img_raw = cv::imread(ppc::util::GetAbsolutePath("all/zaitsev_a_bw_labeling/data/" + filename));
...
TEST_F(zaitsev_a_labeling_test_mpi, validation_fails_on_incorrect_input) {
  const int width = 16;
  ...
  zaitsev_a_labeling_mpi::Labeler intask(task_data_seq);
  if (world.rank() == 0) {
    EXPECT_FALSE(intask.ValidationImpl());
  }
}
#ifndef _WIN32
TEST_P(zaitsev_a_labeling_test_mpi, returns_correct_label_map) {
  ...
  zaitsev_a_labeling_mpi::Labeler task(task_data_seq);
