...
TEST(tyshkevich_a_hoare_simple_merge_all, test_pipeline_run) {
  ...
  auto stt = tyshkevich_a_hoare_simple_merge_all::CreateHoareTestTask<int>(dat, std::greater<>());
...
TEST(tyshkevich_a_hoare_simple_merge_all, test_task_run) {
  auto in = RevSrtVec<int>(kTestSequenceSize);
  std::vector<int> out(in.size());
