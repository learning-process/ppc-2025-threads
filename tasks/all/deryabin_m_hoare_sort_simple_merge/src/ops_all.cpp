...
void deryabin_m_hoare_sort_simple_merge_mpi::MergeUnequalTwoParts(std::vector<double>::iterator first,
    ...
    const size_t overlap_len = static_cast<size_t>(std::min(
        std::distance(left_end, mid),
...
bool deryabin_m_hoare_sort_simple_merge_mpi::HoareSortTaskSequential::RunImpl() {
    ...
    for (size_t i = 0; i < static_cast<size_t>(std::bit_width(chunk_count_)) - 1; ++i) {
        for (size_t j = 0; j < chunk_count; ++j) {
            ...
            std::inplace_merge(
                input_array_A_.begin() + chunk_shift,
                input_array_A_.begin() + mid_point,
...
bool deryabin_m_hoare_sort_simple_merge_mpi::HoareSortTaskMPI::RunImpl() {
    ...
    const auto start_iter = input_array_A_.begin() + static_cast<std::ptrdiff_t>(
        (rank_from_end - 1) * min_chunk_size_ + (world_rank == chunk_count_ - 1 ? 0 : rest_)
    );
    const auto end_iter = input_array_A_.begin() + static_cast<std::ptrdiff_t>(
        rank_from_end * min_chunk_size_ + rest_ - 1
    );
    ...
    for (size_t i = 0; i < iterations; ++i) {
        ...
        if ((chunk_count_ - world_rank) % step == 0 || world_rank == 0) {
            const bool is_even = (chunk_count_ & 1) ? 
                (((chunk_count_ - world_rank + step - 1) / step & 1) != 0) :
            ...
            if (is_even) {
                ...
                if (world_rank >= step) {
                    world_.send(
                        static_cast<int>(world_rank - step), 
                        0, 
                        input_array_A_.data() + start_idx, 
                ...
                MergeUnequalTwoParts(
                    input_array_A_.begin() + static_cast<std::ptrdiff_t>(start_idx),
                    input_array_A_.begin() + static_cast<std::ptrdiff_t>(start_idx + block_size),
                    final_iter
                );
            }
        }
