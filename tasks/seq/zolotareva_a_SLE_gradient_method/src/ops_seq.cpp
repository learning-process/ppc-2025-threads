...
bool zolotareva_a_SLE_gradient_method_seq::TestTaskSequential::ValidationImpl() {
  if (static_cast<int>(task_data->inputs_count[0]) < 0 || static_cast<int>(task_data->inputs_count[1]) < 0 ||
    ...
    return false;
  if (task_data->inputs_count.size() < 2 || task_data->inputs.size() < 2 || task_data->outputs.empty()) return false;
  ...
  if (task_data->outputs_count[0] != task_data->inputs_count[1]) return false;
