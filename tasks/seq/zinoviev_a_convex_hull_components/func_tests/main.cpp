...
void RunAndValidate(const std::vector<int>& input,
  ...
  std::shared_ptr<ppc::core::TaskData> taskData;
  SetupTaskData(input, width, height, expected.size(), taskData);

  zinoviev_a_convex_hull_components_seq::ConvexHullSequential task(taskData);
  ASSERT_TRUE(task.Validation());
  ASSERT_TRUE(task.PreProcessing());
  ASSERT_TRUE(task.Run());
  ASSERT_TRUE(task.PostProcessing());

  auto* output = reinterpret_cast<zinoviev_a_convex_hull_components_seq::Point*>(taskData->outputs[0]);
  std::vector<zinoviev_a_convex_hull_components_seq::Point> actual(output, output + expected.size());

  VerifyResult(actual, expected);
  delete[] reinterpret_cast<zinoviev_a_convex_hull_components_seq::Point*>(taskData->outputs[0]);
}
}  // namespace
