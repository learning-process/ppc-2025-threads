...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, quadratic_3d_integration) {
  ...
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 1.0}, {0.0, 1.0}, {0.0, 1.0}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{170, 170, 170};
  double want = 1.0;
  double tolerance = 10e-3;
  chernykh_a_mir_seq::RunValidTask(func, bounds_per_dim, steps_per_dim, want, tolerance);
}

TEST(chernykh_a_multidimensional_integral_rectangle_seq, exponential_3d_integration) {
  auto func = [](const chernykh_a_mir_seq::Point& point) -> double { return std::exp(point[0] + point[1] + point[2]); };
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 0.5}, {0.0, 0.5}, {0.0, 0.5}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{170, 170, 170};
...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, trigonometric_2d_integration) {
  auto func = [](const chernykh_a_mir_seq::Point& point) -> double { return std::sin(point[0]) * std::cos(point[1]); };
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, std::numbers::pi}, {0.0, std::numbers::pi / 2}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{2000, 2000};
  double want = 2.0;
...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, quadratic_2d_integration) {
  ...
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 2.0}, {0.0, 3.0}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{2000, 2000};
  double want = 26.0;
...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, exponential_2d_integration) {
  auto func = [](const chernykh_a_mir_seq::Point& point) -> double { return std::exp(point[0] + point[1]); };
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 1.0}, {0.0, 1.0}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{2000, 2000};
...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, cubic_2d_integration) {
  ...
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 1.0}, {0.0, 2.0}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{2000, 2000};
  double want = 4.5;
  double tolerance = 10e-3;
  chernykh_a_mir_seq::RunValidTask(func, bounds_per_dim, steps_per_dim, want, tolerance);
...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, empty_bounds_fails_validation) {
  auto func = [](const chernykh_a_mir_seq::Point& point) -> double { return (2 * point[0]) + (3 * point[1]); };
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{2000, 2000};
  chernykh_a_mir_seq::RunInvalidTask(func, bounds_per_dim, steps_per_dim);
}

TEST(chernykh_a_multidimensional_integral_rectangle_seq, incorrect_bounds_fails_validation) {
  auto func = [](const chernykh_a_mir_seq::Point& point) -> double { return std::exp(point[0] + point[1] + point[2]); };
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 0.5}, {1.0, 0.5}, {0.0, 0.5}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{170, 170, 170};
  chernykh_a_mir_seq::RunInvalidTask(func, bounds_per_dim, steps_per_dim);
}
...
TEST(chernykh_a_multidimensional_integral_rectangle_seq, empty_steps_fails_validation) {
  ...
  auto bounds_per_dim = chernykh_a_mir_seq::BoundsPerDim{{0.0, 1.0}, {0.0, 1.0}, {0.0, 1.0}};
  auto steps_per_dim = chernykh_a_mir_seq::StepsPerDim{};
  chernykh_a_mir_seq::RunInvalidTask(func, bounds_per_dim, steps_per_dim);
}
