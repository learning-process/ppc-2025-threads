...
bool kholin_k_multidimensional_integrals_rectangle_seq::TestTaskSequential::PreProcessingImpl() {
  ...
  sz_values = task_data->inputs_count[0];
  sz_lower_limits = task_data->inputs_count[1];
  sz_upper_limits = task_data->inputs_count[2];
  ...
  auto* ptr_start_n = reinterpret_cast<int*>(task_data->inputs[6]);
  start_n = *ptr_start_n;
