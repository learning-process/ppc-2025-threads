...
bool kapustin_i_jarv_alg_seq::TestTaskSequential::ValidationImpl() {
  ...
  do {
    ...
    for (size_t i = 0; i < input_.size(); ++i) {
      ...
      if (orientation > 0) {
        ...
        int dist_next = std::pow(input_[next_index].first - input_[current_index].first, 2) +
                        std::pow(input_[next_index].second - input_[current_index].second, 2);
        int dist_i = std::pow(input_[i].first - input_[current_index].first, 2) +
