...
TEST(FominVConjugateGradientOmp, test_small_system) {
  ...
  fomin_v_conjugate_gradient::FominVConjugateGradientOmp test_task_sequential(task_data_seq);
...
TEST(FominVConjugateGradientOmp, test_large_system) {
...
TEST(FominVConjugateGradientOmp, DotProduct) {
  std::vector<double> a = {1.0, 2.0, 3.0};
  std::vector<double> b = {4.0, 5.0, 6.0};
  double expected = (1.0 * 4.0) + (2.0 * 5.0) + (3.0 * 6.0);  // 32.0
  auto task_data = std::make_shared<ppc::core::TaskData>();
  fomin_v_conjugate_gradient::FominVConjugateGradientOmp task(task_data);
...
TEST(FominVConjugateGradientOmp, VectorAdd) {
  std::vector<double> a = {1.0, 2.0, 3.0};
  std::vector<double> b = {4.0, 5.0, 6.0};
  std::vector<double> expected = {5.0, 7.0, 9.0};
  auto result = fomin_v_conjugate_gradient::FominVConjugateGradientOmp::VectorAdd(a, b);
...
TEST(FominVConjugateGradientOmp, VectorSub) {
  std::vector<double> a = {1.0, 2.0, 3.0};
  std::vector<double> b = {4.0, 5.0, 6.0};
  std::vector<double> expected = {-3.0, -3.0, -3.0};
  auto result = fomin_v_conjugate_gradient::FominVConjugateGradientOmp::VectorSub(a, b);
  EXPECT_EQ(result.size(), expected.size());
  for (size_t i = 0; i < result.size(); ++i) {
    EXPECT_DOUBLE_EQ(result[i], expected[i]);
  }
...
TEST(FominVConjugateGradientOmp, VectorScalarMultiply) {
  std::vector<double> v = {1.0, 2.0, 3.0};
  double scalar = 2.0;
  std::vector<double> expected = {2.0, 4.0, 6.0};
  auto result = fomin_v_conjugate_gradient::FominVConjugateGradientOmp::VectorScalarMultiply(v, scalar);
  EXPECT_EQ(result.size(), expected.size());
  for (size_t i = 0; i < result.size(); ++i) {
    EXPECT_DOUBLE_EQ(result[i], expected[i]);
  }
