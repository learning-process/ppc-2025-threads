...
TEST(naumov_b_marc_on_bin_image_omp, test_pipeline_run) {
  ...
  task_data_omp->inputs_count.emplace_back(kCo);
  task_data_omp->inputs_count.emplace_back(kCo);
  task_data_omp->outputs.emplace_back(reinterpret_cast<uint8_t *>(out.data()));
  task_data_omp->outputs_count.emplace_back(kCo);
  task_data_omp->outputs_count.emplace_back(kCo);

  auto test_task_omp = std::make_shared<naumov_b_marc_on_bin_image_omp::TestTaskOpenMP>(task_data_omp);
