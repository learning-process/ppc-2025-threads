...
TEST(solovev_a_ccs_mmult_sparse_omp, test_pipeline_run) {
  int rows = 2000000;
  int cols = 2000000;
  solovev_a_matrix_omp::MatrixInCcsSparse m1(rows, cols);
  solovev_a_matrix_omp::MatrixInCcsSparse m2(rows, 1);
  solovev_a_matrix_omp::MatrixInCcsSparse m3(rows, 1);
  ...
  auto test_task_omp = std::make_shared<solovev_a_matrix_omp::OMPMatMultCcs>(task_data_omp);
