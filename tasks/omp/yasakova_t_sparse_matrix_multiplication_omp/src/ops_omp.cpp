...
#pragma omp parallel for
  for (int i = 0; i < static_cast<int>(num_rows); ++i) {
    for (uint32_t j = 0; j < num_cols; ++j) {
      ...
      while (left_element_ptr < left_matrix_.row_pointers[i + 1] && right_element_ptr < transposed_right_matrix_.row_pointers[j + 1]) {
        if (left_matrix_.column_indices[left_element_ptr] < transposed_right_matrix_.column_indices[right_element_ptr]) {
          ++left_element_ptr;
        } else if (left_matrix_.column_indices[left_element_ptr] > transposed_right_matrix_.column_indices[right_element_ptr]) {
          ++right_element_ptr;
        } else {
          product_sum += left_matrix_.elements[left_element_ptr++] * transposed_right_matrix_.elements[right_element_ptr++];
        }
      }
      if (product_sum != 0.0) {
