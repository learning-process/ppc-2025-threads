...
struct MatrixStructure {
  ...
  std::complex<double>& AccessElement(size_t row, size_t col) { return elements[(row * num_cols) + col]; }
...
struct SparseMatrixFormat {
  ...
  [[nodiscard]] size_t RowCount() const { return row_pointers.size() - 1; }
  [[nodiscard]] size_t ColumnCount() const { return columns; }
...
inline MatrixStructure MatrixMultiply(MatrixStructure& mat_a, MatrixStructure& mat_b) {
  ...
  for (size_t i = 0; i < mat_a.num_rows; i++) {
    for (size_t j = 0; j < mat_b.num_cols; j++) {
      result.AccessElement(i, j) = 0;
      for (size_t k = 0; k < mat_b.num_rows; k++) {
...
inline MatrixStructure ConvertFromCRS(const SparseMatrixFormat& input_matrix) {
  ...
  for (size_t row = 0; row < matrix.num_rows; ++row) {
    for (size_t i = input_matrix.row_pointers[row]; i < input_matrix.row_pointers[row + 1]; ++i) {
