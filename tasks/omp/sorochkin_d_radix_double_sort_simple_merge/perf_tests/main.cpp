...
void PerfTest(MiddlewareFn middleware) {
  ...
  auto task = std::make_shared<sorochkin_d_radix_double_sort_simple_merge_omp::SortTask>(data);
...
TEST(sorochkin_d_radix_double_sort_simple_merge_omp, test_pipeline_run) {
  PerfTest([](const auto &perf_analyzer, const auto &perf_attr, const auto &perf_results) {
    perf_analyzer.PipelineRun(perf_attr, perf_results);
  });
}

TEST(sorochkin_d_radix_double_sort_simple_merge_omp, test_task_run) {
  PerfTest([](const auto &perf_analyzer, const auto &perf_attr, const auto &perf_results) {
