...
TestData::TestData(std::pair<int, int> matrix1_size, std::pair<int, int> matrix2_size, int sparse_size) {
  ...
  task_data_omp->inputs.emplace_back(reinterpret_cast<uint8_t *>(random_data.data()));
  task_data_omp->inputs.emplace_back(reinterpret_cast<uint8_t *>(single_matrix.data()));
  task_data_omp->inputs_count.emplace_back(matrix1_size.first);
  task_data_omp->inputs_count.emplace_back(matrix1_size.second);
  task_data_omp->inputs_count.emplace_back(matrix2_size.first);
  task_data_omp->inputs_count.emplace_back(matrix2_size.second);
...
lavrentiev_a_ccs_omp::CCSOMP TestData::CreateTask() const { return lavrentiev_a_ccs_omp::CCSOMP(task_data_omp); }
}  // namespace

TEST(lavrentiev_a_ccs_omp, test_0x0_matrix) {
  std::vector<double> a;
  std::vector<double> b;
  std::vector<double> result;
  ...
  lavrentiev_a_ccs_omp::CCSOMP test_task_omp(task_data_omp);
...
TEST(lavrentiev_a_ccs_omp, test_3x2_matrix) {
...
TEST(lavrentiev_a_ccs_omp, test_3x3_matrixes) {
  ...
  task_data_omp->outputs.emplace_back(reinterpret_cast<uint8_t *>(result.data()));
  task_data_omp->outputs_count.emplace_back(result.size());
  lavrentiev_a_ccs_omp::CCSOMP test_task_sequential(task_data_omp);
