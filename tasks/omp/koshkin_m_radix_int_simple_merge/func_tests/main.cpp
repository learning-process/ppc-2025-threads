...
void PerformFuncTest(size_t size) {
  ...
  auto task = koshkin_m_radix_int_simple_merge::OmpT(task_data);
...
#define RANDOM_RND_TEST(size) \
  TEST(koshkin_m_radix_int_simple_merge_omp, random_##size) { PerformFuncTest<kRandom>(size); }
#define RANDOM_REV_TEST(size) \
  TEST(koshkin_m_radix_int_simple_merge_omp, reversed_##size) { PerformFuncTest<kReverse>(size); }
