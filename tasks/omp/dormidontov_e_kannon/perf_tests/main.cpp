...
TEST(dormidontov_e_kannon_omp, test_pipeline_run) {
  ...
  auto task_seq = std::make_shared<dormidontov_e_kannon_omp::OmpTask>(task_data_seq);
...
TEST(dormidontov_e_kannon_omp, test_task_run) {
  size_t test_side_size = 300;
  size_t test_num_blocks = 30;
  std::vector<double> a(test_side_size * test_side_size, 1.0);
  std::vector<double> b(test_side_size * test_side_size, 1.0);
