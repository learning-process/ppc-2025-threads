...
TEST(dormidontov_e_kannon_tbb, test_pipeline_run) {
  ...
  auto task_seq = std::make_shared<dormidontov_e_kannon_tbb::TbbTask>(task_data_seq);
...
TEST(dormidontov_e_kannon_tbb, test_task_run) {
  tbb::global_control global_limit(tbb::global_control::max_allowed_parallelism, 4);
  size_t test_side_size = 300;
  size_t test_num_blocks = 30;
  std::vector<double> a(test_side_size * test_side_size, 1.0);
