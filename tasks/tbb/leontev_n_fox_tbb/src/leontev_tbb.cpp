#include "tbb/leontev_n_fox_tbb/include/leontev_tbb.hpp"
...
namespace leontev_n_fox_tbb {

double FoxTBB::AtA(size_t i, size_t j) const {
  if (i >= n_ || j >= n_) {
    return 0.0;
  }
...
double FoxTBB::AtB(size_t i, size_t j) const {
  if (i >= n_ || j >= n_) {
    return 0.0;
  }
...
void FoxTBB::MatMulBlocks(size_t a_pos_x, size_t a_pos_y, size_t b_pos_x, size_t b_pos_y, size_t c_pos_x,
...
bool FoxTBB::RunImpl() {
  ...
  for (size_t l = 0; l < q; l++) {
    oneapi::tbb::parallel_for(oneapi::tbb::blocked_range2d<size_t>(0, q, 0, q),
...
bool FoxTBB::PostProcessingImpl() {
