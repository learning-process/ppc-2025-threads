...
void TrapezoidalMethodTBB::CreateTaskData(std::shared_ptr<ppc::core::TaskData> &task_data, TaskContext &context,
                                          double *out) {
...
bool TrapezoidalMethodTBB::ValidationImpl() {
...
bool TrapezoidalMethodTBB::PreProcessingImpl() {
  data_ = *reinterpret_cast<TaskContext *>(task_data->inputs[0]);
  return true;
}
bool TrapezoidalMethodTBB::RunImpl() {
  res_ = Integrator<kTBB>{}(data_.function, data_.bounds, data_.precision);
  return true;
}
bool TrapezoidalMethodTBB::PostProcessingImpl() {
