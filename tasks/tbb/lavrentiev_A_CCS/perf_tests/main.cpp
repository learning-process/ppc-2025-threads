...
TEST(lavrentiev_a_ccs_tbb, test_pipeline_run) {
  constexpr double kSize = 700;
  auto task = TestData({kSize, kSize}, {kSize, kSize}, 6);
  auto test_task_tbb = std::make_shared<lavrentiev_a_ccs_tbb::CCSTBB>(task.task_data_tbb);
...
TEST(lavrentiev_a_ccs_tbb, test_task_run) {
  constexpr double kSize = 700;
  auto task = TestData({kSize, kSize}, {kSize, kSize}, 6);
  auto test_task_tbb = std::make_shared<lavrentiev_a_ccs_tbb::CCSTBB>(task.task_data_tbb);
  auto perf_attr = std::make_shared<ppc::core::PerfAttr>();
