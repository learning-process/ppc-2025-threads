...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_opencv_image_validation) {
  ...
  cv::Mat input_image =
      cv::imread(ppc::util::GetAbsolutePath("tbb/varfolomeev_g_histogram_linear_stretching/data/cobble_orig.jpg"),
                 cv::IMREAD_GRAYSCALE);
  ASSERT_FALSE(input_image.empty());

  // loading template modified img
  cv::Mat reference_image =
      cv::imread(ppc::util::GetAbsolutePath("tbb/varfolomeev_g_histogram_linear_stretching/data/cobble_mod.jpg"),
                 cv::IMREAD_GRAYSCALE);
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_opencv_manual_64x64) {
  // loading template orginal img
  cv::Mat input_image =
      cv::imread(ppc::util::GetAbsolutePath("tbb/varfolomeev_g_histogram_linear_stretching/data/cobble_orig.jpg"),
                 cv::IMREAD_GRAYSCALE);
  ...
  task_data_tbb->inputs.emplace_back(reinterpret_cast<uint8_t *>(input_vector.data()));
  task_data_tbb->inputs_count.emplace_back(input_vector.size());
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_manual_10) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_manual_25) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_stretched) {
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_boundary_values) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_flat) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_full_black) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_full_white) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_single_non_flat_pixel) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_10k_generated) {
  // Create data
  const int sz = 10000;
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_empty) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_tbb, test_in_out_NE) {
  // Create data
