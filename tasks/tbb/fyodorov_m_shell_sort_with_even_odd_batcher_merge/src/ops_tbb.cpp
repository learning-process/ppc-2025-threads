...
void TestTaskTBB::ShellSort() {
  ...
  // Вычисляем последовательность промежутков (gaps) для сортировки Шелла
  ...
  // Проходим по промежуткам в обратном порядке
  for (auto it = gaps.rbegin(); it != gaps.rend(); ++it) {
    int gap = *it;

    // Используем TBB для параллельной обработки
    tbb::parallel_for(tbb::blocked_range<int>(0, gap),  // Разделяем работу по базовым индексам
