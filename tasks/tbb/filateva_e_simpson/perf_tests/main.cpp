...
TEST(filateva_e_simpson_tbb, test_pipeline_run) {
  size_t mer = 1;
  size_t steps = 10000000;
  std::vector<double> a = {1};
  std::vector<double> b = {1000};
  std::vector<double> res(1, 0);
  filateva_e_simpson_tbb::Func f = [](std::vector<double> x) {
    if (x.empty()) {
      return 0.0;
    }
    return x[0] * x[0];
  ...
  auto test_task_sequential = std::make_shared<filateva_e_simpson_tbb::Simpson>(task_data);
...
TEST(filateva_e_simpson_tbb, test_task_run) {
  size_t mer = 1;
  size_t steps = 10000000;
  std::vector<double> a = {1};
  std::vector<double> b = {1000};
