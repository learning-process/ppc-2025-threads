...
std::vector<frolova_e_sobel_filter_tbb::RGB> ConvertToRGB(const std::vector<int> &pict) {
  std::vector<frolova_e_sobel_filter_tbb::RGB> picture;
  size_t pixel_count = pict.size() / 3;

  for (size_t i = 0; i < pixel_count; i++) {
    frolova_e_sobel_filter_tbb::RGB pixel;
    pixel.R = pict[i * 3];
    pixel.G = pict[(i * 3) + 1];
    pixel.B = pict[(i * 3) + 2];
...
std::vector<int> ToGrayScaleImg(std::vector<frolova_e_sobel_filter_tbb::RGB> &color_img, size_t width, size_t height) {
...
TEST(frolova_e_sobel_filter_tbb, small_image_1) {
...
TEST(frolova_e_sobel_filter_tbb, test_1) {
...
TEST(frolova_e_sobel_filter_tbb, small_image_2) {
...
TEST(frolova_e_sobel_filter_tbb, _1000_1000_picture) {
  ...
  std::vector<frolova_e_sobel_filter_tbb::RGB> picture = ConvertToRGB(pict);
...
TEST(frolova_e_sobel_filter_tbb, vector_is_not_multiple_of_three) {
  std::vector<int> value = {1, 1};
...
TEST(frolova_e_sobel_filter_tbb, vector_element_is_not_included_the_range) {
  std::vector<int> value = {1, 1};
...
TEST(frolova_e_sobel_filter_tbb, negative_value_of_element_int_RGBvector) {
  std::vector<int> value = {1, 1};
