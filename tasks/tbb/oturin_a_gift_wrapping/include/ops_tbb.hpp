...
class TestTaskTBB::PointSearcher {
 ...
 public:
  ...
  PointSearcher(TestTaskTBB *parent, int search_index) : p(parent), search_index(search_index) {}

  PointSearcher(PointSearcher &x, tbb::split) : p(x.p), search_index(x.search_index) {}

  void operator()(const tbb::blocked_range<int> &r);

  void join(const PointSearcher &x);
};
