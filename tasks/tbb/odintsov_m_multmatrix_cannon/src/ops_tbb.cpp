...
void MulMatrixCannonTBB::MultiplyBlock(int bi, int bj, int root) {
  // Локальные буферы для A, B и результата
  std::vector<double> localA(block_sz_ * block_sz_);
  std::vector<double> localB(block_sz_ * block_sz_);
  std::vector<double> localC(block_sz_ * block_sz_, 0.0);

  // Начальный индекс блока в развернутой матрице
  int start = (bi * block_sz_) * root + (bj * block_sz_);

  // Копируем блоки из глобальных матриц
  CopyBlock(matrixA_, localA, start, root, block_sz_);
  CopyBlock(matrixB_, localB, start, root, block_sz_);

  // Собственно умножение блока A на блок B → localC
  for (int i = 0; i < block_sz_; ++i) {
    for (int k = 0; k < block_sz_; ++k) {
      double a = localA[(i * block_sz_) + k];
      for (int j = 0; j < block_sz_; ++j) {
        localC[(i * block_sz_) + j] += a * localB[(k * block_sz_) + j];
      }
    }
  }
...
bool MulMatrixCannonTBB::RunImpl() {
